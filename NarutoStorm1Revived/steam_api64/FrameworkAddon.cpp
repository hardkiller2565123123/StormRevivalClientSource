...
namespace
{
    ...
    std::string g_Name = "NS1FrameWork.dll";
    std::string g_Path;
    ...
    FARPROC RequiredProc(const char* name)
    {
        ...
        if (!proc)
            Logger::Error(std::string("NS1FrameWork.dll missing export ") + name);
        return proc;
    }

    std::filesystem::path FrameworkPath()
    {
        return std::filesystem::path(ExeFolder()) / "NarutoStorm1Revived" / "Tools" / "NS1FrameWork.dll";
    }
}
...
namespace FrameworkAddon
{
    ...
    bool Load()
    {
        ...
        if (!g_Module)
        {
            g_Status = "NS1FrameWork.dll is present, but Windows could not load it.";
            return false;
        }
        ...
        {
            g_Status = "NS1FrameWork.dll loaded, but its API is incomplete.";
        ...
        if (apiVersion != 1)
        {
            g_Status = "NS1FrameWork.dll API version mismatch.";
            FreeLibrary(g_Module);
            g_Module = nullptr;
            g_DrawGui = nullptr;
            Logger::Error(g_Status);
    ...
    void DrawClientPanel()
    {
        ...
        {
            ImGui::Separator();
            ImGui::TextWrapped("NS1FrameWork.dll is optional. The base client and online features work without it.");
            return;
        }
