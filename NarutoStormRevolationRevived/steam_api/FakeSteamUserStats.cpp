...
namespace
{
    constexpr uint64_t kOfflineGameID = 272510;
    ...
    std::string LeaderboardPath()
    {
        ...
        std::filesystem::path out = base.parent_path() / "NarutoStormRevolationRevived" / "leaderboards.ini";
    ...
    {
        std::string value = name && name[0] ? name : "NSRRevivedLeaderboard";
    ...
    std::vector<LeaderboardEntryStub> SnapshotLeaderboardRows(SteamLeaderboard_t leaderboard)
    {
        ...
        {
            const std::string fallback = "NSRRevivedLeaderboard";
    ...
    std::string LeaderboardNameForHandle(SteamLeaderboard_t leaderboard)
    {
        ...
        return it == g_LeaderboardNames.end() ? "NSRRevivedLeaderboard" : it->second;
    }
...
public:
    ...
    virtual const char* GetLeaderboardName(SteamLeaderboard_t leaderboard)
    {
        ...
        static thread_local std::string s_Name;
        {
            ...
            s_Name = it == g_LeaderboardNames.end() ? "NSRRevivedLeaderboard" : it->second;
        }
