...
namespace
{
    ...
    constexpr AppId_t kNSRAppId = 272510;
...
#pragma pack(pop)
    ...
    void* RouteVersion(const char* version)
    {
        ...
        if (HasInterface(u, "STEAMNETWORKINGMESSAGES")) return NSRRevived_SteamNetworkingMessages();
        if (HasInterface(u, "STEAMNETWORKINGSOCKETS")) return NSRRevived_SteamNetworkingSockets();
        if (HasInterface(u, "STEAMNETWORKINGUTILS")) return NSRRevived_SteamNetworkingUtils();
        if (HasInterface(u, "STEAMNETWORKING")) return NSRRevived_SteamNetworking();
    ...
    public:
        ...
        {
            NSR_UNUSED(appID);
            if (depots && maxDepots > 0) depots[0] = kNSRAppId;
    ...
    private:
        ...
        std::string m_Product = "NarutoStormRevolationRevived";
    ...
    public:
        ...
        virtual uint32_t GetAppID() { TraceCore("SteamUtils", "GetAppID"); return kNSRAppId; }
    ...
    public:
        ...
        virtual void* GetISteamNetworking(HSteamUser user, HSteamPipe pipe, const char* version) { SteamVersionLogger::LogInterfaceRequest("OfflineSteamClient::GetISteamNetworking", version); NSR_UNUSED(user); NSR_UNUSED(pipe); return NSRRevived_SteamNetworking(); }
...
namespace FakeSteamCore
{
    bool Init()
    {
        ...
        g_UserDataFolder = (std::filesystem::path(g_InstallPath) / "NarutoStormRevolationRevived" / "userdata").string();
