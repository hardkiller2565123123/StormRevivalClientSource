...
namespace
{
    ...
    std::string FindBestDataFolder()
    {
        ...
        std::vector<std::filesystem::path> candidates =
        {
            ...
            game / "disc" / "data",
            game / "native" / "data",
            game / "NarutoStormRevolationRevived" / "data"
        };
...
namespace AssetBrowser
{
    bool Init()
    {
        ...
        g_ModsFolder = (std::filesystem::path(g_GameFolder) / "NarutoStormRevolationRevived" / "mods").string();
        g_DumpFolder = (std::filesystem::path(g_GameFolder) / "NarutoStormRevolationRevived" / "Dump").string();
        g_ToolsFolder = (std::filesystem::path(g_GameFolder) / "NarutoStormRevolationRevived" / "Tools").string();
        g_CacheFolder = (std::filesystem::path(g_GameFolder) / "NarutoStormRevolationRevived" / "Cache").string();
