...
namespace
{
    ...
    char g_ModDescription[256] = "Created with NSR Revived V2.";
    ...
    void DrawTopToolbar(const std::vector<AssetBrowser::AssetEntry>& assets)
    {
        ...
        {
            AssetTaskQueue::EnqueueExportCsv("NarutoStormRevolationRevived/Dump/asset_index.csv");
            AssetNotification::PushInfo("Report", "CSV export queued.");
        }

        if (wrappedButton("Full Report", 100.0f))
        {
            AssetReport::ExportFullReport("NarutoStormRevolationRevived/Dump/full_asset_report.txt");
    ...
    void DrawToolsPage()
    {
        ...
        if (ImGui::Button("Export Dependency Report", ImVec2(210, 24)))
            AssetReport::ExportDependencyReport("NarutoStormRevolationRevived/Dump/dependency_report.txt");
