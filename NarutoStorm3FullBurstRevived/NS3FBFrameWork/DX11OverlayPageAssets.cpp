...
namespace
{
    ...
    char g_ModDescription[256] = "Created with NS3 Full Burst Revived V2.";
    ...
    void DrawTopToolbar(const std::vector<AssetBrowser::AssetEntry>& assets)
    {
        ...
        {
            AssetTaskQueue::EnqueueExportCsv("NarutoStorm3FullBurstRevived/Dump/asset_index.csv");
            AssetNotification::PushInfo("Report", "CSV export queued.");
        }

        if (wrappedButton("Full Report", 100.0f))
        {
            AssetReport::ExportFullReport("NarutoStorm3FullBurstRevived/Dump/full_asset_report.txt");
    ...
    void DrawToolsPage()
    {
        ...
        if (ImGui::Button("Export Dependency Report", ImVec2(210, 24)))
            AssetReport::ExportDependencyReport("NarutoStorm3FullBurstRevived/Dump/dependency_report.txt");
