...
namespace
{
    ...
    std::string g_Name = "NS3FBFrameWork.dll";
    std::string g_Path;
    ...
    FARPROC RequiredProc(const char* name)
    {
        ...
        if (!proc)
            Logger::Error(std::string("NS3FBFrameWork.dll missing export ") + name);
        return proc;
    }

    std::filesystem::path FrameworkPath()
    {
        return std::filesystem::path(ExeFolder()) / "NarutoStorm3FullBurstRevived" / "Tools" / "NS3FBFrameWork.dll";
    }
}
...
namespace FrameworkAddon
{
    ...
    bool Load()
    {
        ...
        if (!g_Module)
        {
            g_Status = "NS3FBFrameWork.dll is present, but Windows could not load it.";
            return false;
        }
        ...
        {
            g_Status = "NS3FBFrameWork.dll loaded, but its API is incomplete.";
        ...
        if (apiVersion != 1)
        {
            g_Status = "NS3FBFrameWork.dll API version mismatch.";
            FreeLibrary(g_Module);
            g_Module = nullptr;
            g_DrawGui = nullptr;
            Logger::Error(g_Status);
    ...
    void DrawClientPanel()
    {
        ...
        {
            ImGui::Separator();
            ImGui::TextWrapped("NS3FBFrameWork.dll is optional. The base client and online features work without it.");
            return;
        }
