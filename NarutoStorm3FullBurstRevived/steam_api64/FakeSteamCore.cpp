...
namespace
{
    ...
    constexpr AppId_t kNS3FBAppId = 234670;
...
#pragma pack(pop)
    ...
    void* RouteVersion(const char* version)
    {
        ...
        if (HasInterface(u, "STEAMNETWORKINGMESSAGES")) return NS3FBRevived_SteamNetworkingMessages();
        if (HasInterface(u, "STEAMNETWORKINGSOCKETS")) return NS3FBRevived_SteamNetworkingSockets();
        if (HasInterface(u, "STEAMNETWORKINGUTILS")) return NS3FBRevived_SteamNetworkingUtils();
        if (HasInterface(u, "STEAMNETWORKING")) return NS3FBRevived_SteamNetworking();
    ...
    public:
        ...
        {
            NSR_UNUSED(appID);
            if (depots && maxDepots > 0) depots[0] = kNS3FBAppId;
    ...
    private:
        ...
        std::string m_Product = "NarutoStorm3FullBurstRevived";
    ...
    public:
        ...
        virtual uint32_t GetAppID() { TraceCore("SteamUtils", "GetAppID"); return kNS3FBAppId; }
    ...
    public:
        ...
        virtual void* GetISteamNetworking(HSteamUser user, HSteamPipe pipe, const char* version) { SteamVersionLogger::LogInterfaceRequest("OfflineSteamClient::GetISteamNetworking", version); NSR_UNUSED(user); NSR_UNUSED(pipe); return NS3FBRevived_SteamNetworking(); }
...
namespace FakeSteamCore
{
    bool Init()
    {
        ...
        g_UserDataFolder = (std::filesystem::path(g_InstallPath) / "NarutoStorm3FullBurstRevived" / "userdata").string();
