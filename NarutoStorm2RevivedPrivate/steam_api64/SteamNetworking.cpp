...
static const char* NS2_DEFAULT_P2P_HOST = "127.0.0.1";
static const uint16_t NS2_DEFAULT_P2P_PORT = 47584;
...
private:
    bool EnsureSocket()
    {
        ...
        if (host.empty())
            host = NS2_DEFAULT_P2P_HOST;

        if (port == 0)
            port = NS2_DEFAULT_P2P_PORT;
        ...
        {
            Logger::Error("NS2SteamNetworking: invalid custom P2P host, falling back to 127.0.0.1");
            host = NS2_DEFAULT_P2P_HOST;
            inet_pton(AF_INET, host.c_str(), &serverAddress.sin_addr);
        }

        Logger::Info(
            "NS2SteamNetworking: custom LAN/Radmin endpoint " +
            host +
            ":" +
            std::to_string(port));
...
public:
        ...
        P2PSessionState_t* state)
    {
        ...
        endpoint.sin_port = htons(port ? port : NS2_DEFAULT_P2P_PORT);

        if (host == "localhost" || host.empty())
            host = NS2_DEFAULT_P2P_HOST;

        if (inet_pton(AF_INET, host.c_str(), &endpoint.sin_addr) != 1)
            inet_pton(AF_INET, NS2_DEFAULT_P2P_HOST, &endpoint.sin_addr);
        ...
        int timeout)
    {
        ...
        Logger::Info("NS2SteamNetworking: CreateConnectionSocket");
        return 1;
    }
        ...
        bool reliable)
    {
        ...
        Logger::Info(
            "NS2SteamNetworking: SendDataOnSocket size=" +
            std::to_string(size));
        return true;
    }
        ...
        uint16_t* remotePort)
    {
        ...
        SafeWriteValue(remotePort, NS2_DEFAULT_P2P_PORT);
...
{
    SteamVersionLogger::LogInterfaceRequest("NS2Revived_SteamNetworking", "SteamNetworking006");
    return &g_Networking;
}

extern "C" void* __cdecl NS2Revived_SteamGameServerNetworking()
{
    SteamVersionLogger::LogInterfaceRequest("NS2Revived_SteamGameServerNetworking", "SteamNetworking006");
    return &g_Networking;
}

extern "C" void* __cdecl NS2Revived_SteamNetworkingMessages()
{
    SteamVersionLogger::LogInterfaceRequest("NS2Revived_SteamNetworkingMessages", "SteamNetworkingMessages002");
    return &g_Messages;
}

extern "C" void* __cdecl NS2Revived_SteamNetworkingSockets()
{
    SteamVersionLogger::LogInterfaceRequest("NS2Revived_SteamNetworkingSockets", "SteamNetworkingSockets012");
    return &g_Sockets;
}

extern "C" void* __cdecl NS2Revived_SteamGameServerNetworkingSockets()
{
    SteamVersionLogger::LogInterfaceRequest("NS2Revived_SteamGameServerNetworkingSockets", "SteamNetworkingSockets012");
    return &g_Sockets;
}

extern "C" void* __cdecl NS2Revived_SteamNetworkingUtils()
{
    SteamVersionLogger::LogInterfaceRequest("NS2Revived_SteamNetworkingUtils", "SteamNetworkingUtils004");
    return &g_Utils;
}
