...
namespace
{
    ...
    public:
        ...
        {
            strncpy_s(m_szServerName, name ? name : "NS2 Revived Offline Server", _TRUNCATE);
        }
        ...
        uint32_t m_nAppID = kStorm2AppId;
        int m_nPlayers = 1;
        int m_nMaxPlayers = 4;
        int m_nBotPlayers = 0;
    ...
    gameserveritem_t MakeServerItem(uint32_t ip, uint16_t port)
    {
        ...
        lobby.Name = "NS2 Revived Direct Server";
    ...
    public:
        ...
        virtual HServerQuery PlayerDetails(uint32_t ip, uint16_t port, ISteamMatchmakingPlayersResponse* response)
        {
            ...
            if (response && HasReadableVTable(response, 3))
            {
                response->AddPlayerToList("NS2Revived", 0, 0.0f);
