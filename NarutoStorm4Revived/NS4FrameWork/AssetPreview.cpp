...
namespace
{
    ...
    void FillXfbinInfo(const std::string& path, AssetPreview::PreviewState& state)
    {
        ...
        state.Title = "XFBIN/XBIN Info";
        state.Bytes = b;

        std::ostringstream ss;
        ss << "XFBIN/XBIN Quick Viewer\n";
        ss << "Path: " << path << "\n";
        ...
        const char* terms[] = { "XBIN", "xbin", "nuccChunk", "nucc", "NUCC", "texture", "material", "model", "stage", "player", "camera", "anm" };
        ...
        state.Text = ss.str();
        state.Valid = true;
        state.Status = "XFBIN/XBIN metadata scan loaded";
    }

    void FillGfxInfo(const std::string& path, AssetPreview::PreviewState& state)
    {
        std::vector<unsigned char> b;
        AssetPreview::ReadBytes(path, b, 1024 * 1024);
        state.Kind = AssetPreview::PreviewKind::GfxInfo;
        state.Title = "GFX / Scaleform Info";
        state.Bytes = b;

        std::ostringstream ss;
        ss << "GFX / Scaleform Quick Viewer\n";
        ss << "Path: " << path << "\n";
        ss << "Bytes loaded: " << b.size() << "\n";

        if (b.size() >= 3)
        {
            std::string sig(reinterpret_cast<const char*>(b.data()), reinterpret_cast<const char*>(b.data()) + 3);
            ss << "Signature: " << sig << "\n";

            if (sig == "FWS")
                ss << "Container: uncompressed SWF-compatible Scaleform data\n";
            else if (sig == "CWS")
                ss << "Container: compressed SWF-compatible Scaleform data\n";
            else if (sig == "GFX")
                ss << "Container: Scaleform GFX\n";
        }

        ss << "\nDetected UI strings / render markers:\n";
        std::string text(reinterpret_cast<const char*>(b.data()), reinterpret_cast<const char*>(b.data()) + b.size());
        const char* terms[] = { "Scaleform", "GFx", "Movie", "Sprite", "TextField", "Button", "Renderer", "Texture", "Font", "ActionScript" };
        std::vector<std::string> found;

        for (const char* t : terms)
        {
            size_t pos = 0;
            while ((pos = text.find(t, pos)) != std::string::npos && found.size() < 160)
            {
                size_t begin = pos;
                size_t end = pos;

                while (begin > 0 && static_cast<unsigned char>(text[begin - 1]) >= 32 && static_cast<unsigned char>(text[begin - 1]) < 127)
                    --begin;

                while (end < text.size() && static_cast<unsigned char>(text[end]) >= 32 && static_cast<unsigned char>(text[end]) < 127)
                    ++end;

                std::string marker = text.substr(begin, end - begin);
                if (!marker.empty() && std::find(found.begin(), found.end(), marker) == found.end())
        ...
        ss << "\nHeader Hex\n";
        ss << AssetPreview::BuildHex(b, 512);

        state.Text = ss.str();
        state.Valid = true;
        state.Status = "GFX metadata scan loaded";
    }
}
