...
namespace
{
    constexpr const char* kAssetCacheHeader = "NS4ASSETCACHE|3";

    std::string g_GameFolder;
    std::string g_DataFolder;
    std::string g_ModsFolder;
    std::string g_DlcFolder;
    ...
    void ScanInsideLooseFile(AssetBrowser::AssetEntry& asset)
    {
        ...
        if (asset.Extension == ".xfbin" || asset.Extension == ".xbin")
    ...
    void ScanLooseRootOnly(
        const std::filesystem::path& root,
        const std::string& sourceLabel,
        int progressBase,
        int looseProgressSpan,
        int cpkProgressSpan)
    {
        Logger::Info("AssetBrowser " + sourceLabel + " root = " + root.string());

        if (!FileExists(root))
        {
            Logger::Info("AssetBrowser " + sourceLabel + " folder not found: " + root.string());
            return;
        }

        SetScanStatus("Scanning " + sourceLabel + " folder: " + root.string());
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(root, ec))
        {
            ...
            if (ec)
            {
                Logger::Error("AssetBrowser " + sourceLabel + " recursive scan error: " + ec.message());
                break;
            }
        ...
        Logger::Info("AssetBrowser found files in " + sourceLabel + " folder: " + std::to_string(files.size()));
        ...
        for (size_t i = 0; i < files.size(); ++i)
        {
            ...
            SetScanStatus("Indexing " + sourceLabel + ": " + file.filename().string());

            AssetBrowser::AssetEntry asset{};

            asset.Name = file.filename().string();
            asset.FullPath = file.string();
            asset.RelativePath = RelativeTo(file, root);
            asset.VirtualPath = NormalizeSlashes(asset.RelativePath);
            asset.Extension = ToLower(file.extension().string());
            asset.Type = DetectTypeFromPath(asset.RelativePath);
            asset.Source = AssetBrowser::AssetSource::Game;
            ...
            if (looseProgressSpan > 0)
            {
                g_ScanProgress = progressBase + static_cast<int>((static_cast<double>(i + 1) / static_cast<double>(files.size())) * static_cast<double>(looseProgressSpan));
            }
        }
...

        {
            ...
            {
                if (asset.Extension == ".cpk" && !asset.IsCpkEntry && StartsWithNoCase(asset.FullPath, root.string()))
                    packages.push_back(asset);
            }
        }

        Logger::Info("AssetBrowser found " + sourceLabel + " CPK files: " + std::to_string(packages.size()));
        ...
        for (size_t i = 0; i < packages.size(); ++i)
        {
            ...
            SetScanStatus("Indexing " + sourceLabel + " CPK: " + packages[i].Name);
            ...
            if (cpkProgressSpan > 0)
            {
                g_ScanProgress = progressBase + looseProgressSpan + static_cast<int>((static_cast<double>(i + 1) / static_cast<double>(packages.size())) * static_cast<double>(cpkProgressSpan));
            }
        }
    }

    void ScanDlcFolderOnly()
    {
        ScanLooseRootOnly(g_DlcFolder, "DLC", 90, 1, 1);
    }
...
namespace AssetBrowser
{
    bool Init()
    {
        ...
        g_ModsFolder = (std::filesystem::path(g_GameFolder) / "NarutoStorm4Revived" / "mods").string();
        g_DlcFolder = (std::filesystem::path(g_GameFolder) / "dlc").string();
        g_DumpFolder = (std::filesystem::path(g_GameFolder) / "NarutoStorm4Revived" / "Dump").string();
        g_ToolsFolder = (std::filesystem::path(g_GameFolder) / "NarutoStorm4Revived" / "Tools").string();
        g_CacheFolder = (std::filesystem::path(g_GameFolder) / "NarutoStorm4Revived" / "Cache").string();
        ...
        Logger::Info("AssetBrowser DLC scan folder: " + g_DlcFolder);
    ...
    bool LoadCache()
    {
        ...
        while (std::getline(file, line))
        {
            ...
            if (!hasCurrentHeader)
            {
                Logger::Info("AssetBrowser cache is old; forcing a fresh asset scan so new file types show up.");
                return false;
            }
    ...
    bool DumpXfbinInfo(const AssetEntry& asset)
    {
        ...
        try
        {
            ...
            Logger::Info("AssetBrowser wrote XFBIN/XBIN quick info: " + outPath.string());
            return true;
        }
        catch (const std::exception& e)
        {
            Logger::Error(std::string("AssetBrowser XFBIN/XBIN info failed: ") + e.what());
            return false;
        }
    }
