...
namespace
{
	...
	bool originalMsgInfoCaptured = false;
	bool originalMsgInfo2Captured = false;

	void CaptureMessageInfoOriginals()
	{
		if (!originalMsgInfoCaptured && HookFunctions::fc_msgtostring > 0)
		{
			memcpy(originalMsgInfo, (void*)(moduleBase + HookFunctions::fc_msgtostring), sizeof(originalMsgInfo));
			originalMsgInfoCaptured = true;
		}

		if (!originalMsgInfo2Captured && HookFunctions::fc_msgtostring_3 > 0)
		{
			memcpy(originalMsgInfo2, (void*)(moduleBase + HookFunctions::fc_msgtostring_3), sizeof(originalMsgInfo2));
			originalMsgInfo2Captured = true;
		}
	}
}
...
{
	CaptureMessageInfoOriginals();
	HookFunctions::Hook((void*)(moduleBase + fc_msgtostring), (void*)moddingApi::ccGeneralGameFunctions::Hook_MsgToString, length);
}
...
void HookFunctions::UndoMessageInfoHook(int length)
{
	...
	memcpy((void*)(moduleBase + fc_msgtostring), originalMsgInfo, std::min<size_t>(length, sizeof(originalMsgInfo)));
...
{
	CaptureMessageInfoOriginals();
	HookFunctions::Hook((void*)(moduleBase + fc_msgtostring_3), (void*)moddingApi::ccGeneralGameFunctions::Hook_MsgToString_Alt, length);
}
...
void HookFunctions::UndoMessageInfoHook2(int length)
{
	...
	memcpy((void*)(moduleBase + fc_msgtostring_3), originalMsgInfo2, std::min<size_t>(length, sizeof(originalMsgInfo2)));
