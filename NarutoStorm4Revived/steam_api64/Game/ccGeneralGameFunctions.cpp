...
namespace
{
	char* WritableMessage(const char* text)
	{
		return const_cast<char*>(text);
	}
}

char * moddingApi::ccGeneralGameFunctions::MessageToString(char * msg)
{
...
char moddingApi::ccGeneralGameFunctions::ViewMessageConversions = 0;

uintptr_t moddingApi::ccGeneralGameFunctions::Hook_MsgToString(uintptr_t MessageToDecode)
{
	if (ccGeneralGameFunctions::ViewMessageConversions == 0 && strlen((char*)MessageToDecode) > 0 && *(char*)MessageToDecode != '+')
	{
		...
		if (MessageToDecode != 0)
		{
			...
			if (finished == false)
			{
				...
				if (msg == "network_agreement_select") result = WritableMessage("Press any button to continue");
				else if (msg == "network_agreement_ok") result = WritableMessage("");
				else if (msg == "network_agreement_ng") result = WritableMessage("<icon btn_2 />");
				else
				{
...
uintptr_t moddingApi::ccGeneralGameFunctions::Hook_MsgToString_Alt(uintptr_t MessageToDecode)
{
