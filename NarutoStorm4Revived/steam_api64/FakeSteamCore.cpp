...
namespace
{
    ...
    constexpr AppId_t kNS4AppId = 349040;
...
#pragma pack(pop)
    ...
    void* RouteVersion(const char* version)
    {
        ...
        if (HasInterface(u, "STEAMNETWORKINGMESSAGES")) return NS4Revived_SteamNetworkingMessages();
        if (HasInterface(u, "STEAMNETWORKINGSOCKETS")) return NS4Revived_SteamNetworkingSockets();
        if (HasInterface(u, "STEAMNETWORKINGUTILS")) return NS4Revived_SteamNetworkingUtils();
        if (HasInterface(u, "STEAMNETWORKING")) return NS4Revived_SteamNetworking();
    ...
    public:
        ...
        {
            NSR_UNUSED(appID);
            if (depots && maxDepots > 0) depots[0] = kNS4AppId;
    ...
    private:
        ...
        std::string m_Product = "NarutoStorm4Revived";
    ...
    public:
        ...
        virtual uint32_t GetAppID() { TraceCore("SteamUtils", "GetAppID"); return kNS4AppId; }
    ...
    public:
        ...
        virtual void* GetISteamNetworking(HSteamUser user, HSteamPipe pipe, const char* version) { SteamVersionLogger::LogInterfaceRequest("OfflineSteamClient::GetISteamNetworking", version); NSR_UNUSED(user); NSR_UNUSED(pipe); return NS4Revived_SteamNetworking(); }
...
namespace FakeSteamCore
{
    bool Init()
    {
        ...
        g_UserDataFolder = (std::filesystem::path(g_InstallPath) / "NarutoStorm4Revived" / "userdata").string();
