...
namespace
{
    ...
    std::string FindBestDataFolder()
    {
        ...
        std::vector<std::filesystem::path> candidates =
        {
            ...
            game / "disc" / "data",
            game / "native" / "data",
            game / "NarutoStormConnectionsRevived" / "data"
        };
...
namespace AssetBrowser
{
    bool Init()
    {
        ...
        g_ModsFolder = (std::filesystem::path(g_GameFolder) / "NarutoStormConnectionsRevived" / "mods").string();
        g_DumpFolder = (std::filesystem::path(g_GameFolder) / "NarutoStormConnectionsRevived" / "Dump").string();
        g_ToolsFolder = (std::filesystem::path(g_GameFolder) / "NarutoStormConnectionsRevived" / "Tools").string();
        g_CacheFolder = (std::filesystem::path(g_GameFolder) / "NarutoStormConnectionsRevived" / "Cache").string();
